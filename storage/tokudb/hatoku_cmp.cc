...
{
    CHARSET_INFO* charset = get_charset_from_num(charset_number);
#if 80000 <= MYSQL_VERSION_ID && MYSQL_VERSION_ID <= 80099
    int ret_val = charset->coll->strnncollsp(charset, a_buf, a_num_bytes, b_buf, b_num_bytes);
#else
    int ret_val = charset->coll->strnncollsp(charset, a_buf, a_num_bytes, b_buf, b_num_bytes, 0);
#endif
    return ret_val;
}
...
#if TOKU_INCLUDE_ALTER_55 || TOKU_INCLUDE_ALTER_56
